...
def stop_django_processes():  # zatrzymanie procesow servera django
    """This function stops all django server processes"""
    django_procs = find_django_processes()
    for proc in django_procs:
        # print(f"Zatrzymywanie procesu PID {proc.pid}, komenda: {' '.join(proc.info['cmdline'])}")
        proc.kill()
...
# (serverstdout, serverstderr) = run_server.communicate()
time.sleep(5)
